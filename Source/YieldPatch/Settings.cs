...
namespace YieldPatch
{
    ...
    internal class YieldPatchSettings : ModSettings
    {
        ...
        {
            base.ExposeData();
            Scribe_Collections.Look(ref modifiedThings, "modifiedThings", LookMode.Value, LookMode.Value, ref modifiedThingsKeys, ref modifiedThingsValues);
            Scribe_Collections.Look(ref modifiedMass, "modifiedMass", LookMode.Value, LookMode.Value, ref modifiedMassKeys, ref modifiedMassValues);
        }
        ...
        public void DoSettingsWindowContents(Rect inRect)
        {
            ...
            for (var num = keys.Count - 1; num >= 0; num--)
            {
                ...
                listingStandard.AddLabeledSlider($"{GenText.CapitalizeFirst(thing.label)}: {Math.Round(thing.building.mineableYield * value, 0)}", ref value, 0, maxYieldValue, value.ToString(), null, 0.01f);
            ...
            for (var num = keys.Count - 1; num >= 0; num--)
            {
                ...
                listingStandard.AddLabeledSlider($"{GenText.CapitalizeFirst(thing.label)}: {Math.Max(lowValue / 10, thing.BaseMass)}", ref value, lowValue, maxMassValue, value.ToString(), null, 0.01f);
